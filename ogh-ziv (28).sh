...
# ════════════════════════════════════════════════════════════
main_menu() {
    # Lebar tiap kolom = 27 karakter (termasuk spasi padding)
    # _row2: $1=label kiri, $2=label kanan — TANPA warna di parameter,
    #        warna di-inject langsung di dalam fungsi via $3/$4 (color code kiri/kanan)
    # Gunakan _row2c untuk kontrol warna penuh
    _row2c() {
        # $1=warna_kiri $2=teks_kiri $3=warna_kanan $4=teks_kanan
        local CL="$1" TL="$2" CR="$3" TR="$4"
        local pad_l=$(( 25 - ${#TL} )); [[ $pad_l -lt 0 ]] && pad_l=0
        local pad_r=$(( 25 - ${#TR} )); [[ $pad_r -lt 0 ]] && pad_r=0
        printf "  ${A1}║${NC} ${CL}${TL}${NC}%*s ${A1}║${NC} ${CR}${TR}${NC}%*s ${A1}║${NC}\n" \
               "$pad_l" "" "$pad_r" ""
    }

    # Garis border 2 kolom — masing-masing 27 karakter
    ...
    while true; do
        ...
        # ── 2-Column Grid ─────────────────────────────────────
        _top2
        _row2c "${A2}" "[1] Kelola Akun" "${A2}" "[2] Service"
        _sep2d
        _row2c "${A2}" "[3] Telegram Bot" "${A2}" "[4] Menu Jualan"
        _sep2d
        _row2c "${A2}" "[5] Bandwidth" "${A2}" "[6] Restart"
        _sep2d
        _row2c "${A2}" "[7] Install" "${A2}" "[8] Domain"
        _sep2d
        _row2c "${A2}" "[9] Tema [${THEME_NAME}]" "${LR}" "[E] Uninstall"
        _sep2
        _row2c "${A4}" "[0] Keluar" "${DIM}" "OGH-ZIV v1.0"
        _bot2
