...
# ════════════════════════════════════════════════════════════
main_menu() {
    # Helper: cetak 1 baris 2 kotak berdampingan
    # $1 = isi kotak kiri (raw dengan warna), $2 = isi kotak kanan
    _row2() {
        local L="$1" R="$2"
        local llen; llen=$(_displen "$L")
        local rlen; rlen=$(_displen "$R")
        local lpad=$(( 26 - llen )); [[ $lpad -lt 0 ]] && lpad=0
        local rpad=$(( 26 - rlen )); [[ $rpad -lt 0 ]] && rpad=0
        printf "  ${A1}║${NC}%b%*s${A1}║${NC}%b%*s${A1}║${NC}\n" \
               "$L" "$lpad" "" "$R" "$rpad" ""
    }
    # Garis pemisah antar baris di dalam 2 kotak
    _sep2() {
        echo -e "  ${A1}╠$(printf '═%.0s' {1..27})╬$(printf '═%.0s' {1..27})╣${NC}"
    }
    _sep2d() {
        echo -e "  ${A1}╠$(printf '╌%.0s' {1..27})╫$(printf '╌%.0s' {1..27})╣${NC}"
    }
    ...
    _mid2() {  # garis tengah horisontal penuh
        echo -e "  ${A1}╠$(printf '═%.0s' {1..27})╩$(printf '═%.0s' {1..27})╣${NC}"
    }
    ...
    while true; do
        ...
        # ── 2-Column Grid ─────────────────────────────────────
        _top2
        _row2 "  ${A2}[1]${NC}  👤  Kelola Akun" "  ${A2}[2]${NC}  ⚙️   Service"
        _sep2d
        _row2 "  ${A2}[3]${NC}  🤖  Telegram Bot" "  ${A2}[4]${NC}  🛒  Menu Jualan"
        _sep2d
        _row2 "  ${A2}[5]${NC}  📊  Bandwidth" "  ${A2}[6]${NC}  🔄  Restart"
        _sep2d
        _row2 "  ${A2}[7]${NC}  🚀  Install" "  ${A2}[8]${NC}  🌐  Domain"
        _sep2d
        _row2 "  ${A2}[9]${NC}  🎨  Tema ${DIM}[${THEME_NAME}]${NC}" "  ${LR}[E]${NC}  🗑️   Uninstall"
        _sep2
        _row2 "  ${A4}[0]${NC}  ✖   Keluar" "  ${DIM}OGH-ZIV v1.0${NC}"
        _bot2
