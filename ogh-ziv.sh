...
# ════════════════════════════════════════════════════════════
main_menu() {
    # Lebar 1 kolom = 27 karakter (termasuk 1 spasi padding kanan)
    # _cell: $1=warna, $2=emoji(2-wide), $3=teks_ascii, $4=pad_extra
    # Rumus: 1(spasi) + 2(emoji) + len(teks) + pad = 27
    # → pad = 27 - 1 - 2 - len(teks) - 1(spasi sebelum ║) = 23 - len(teks)
    # Tapi kita pakai format:  " CLR[N]NC  EMJ  TXT<pad> "
    # Format tetap: "  [X]  " = 7 char, emoji = 2 wide, "  " = 2, teks, pad, " "
    # Jadi kita hitung pad = 27 - 7 - 2 - 2 - len(teks) - 1 = 15 - len(teks)

    # _cell: cetak satu sel kolom tanpa border kanan
    # $1=color_num $2=label "[N]" $3=emoji $4=label_teks
    _cell() {
        local col="$1" lbl="$2" emj="$3" txt="$4"
        local pad=$(( 14 - ${#txt} ))
        [[ $pad -lt 0 ]] && pad=0
        printf "${col}%s${NC}  %s  %s%*s" "$lbl" "$emj" "$txt" "$pad" ""
    }

    # _row2m: cetak 1 baris 2 kolom — masing2 sel sudah disiapkan oleh _cell
    # Tapi lebih simpel: langsung cetak dengan printf + manual pad
    # Format per kolom: " CLR[N]NC  EMJ  TXT" + spasi sampai 26 char + " "
    # Total per kolom termasuk border = 27+1 = 28? Kita ikuti _top2 = 27 char/kolom

    # Pendekatan paling akurat: hardcode pad per baris
    # Kolom lebar = 27. Format: spasi(2) + "[N]"(3) + spasi(2) + emoji(2vis) + spasi(2) + teks + pad + spasi(1)
    # = 2+3+2+2+2+len(teks)+pad+1 = 14+len(teks)+pad = 27 → pad = 13-len(teks)

    _row2() {
        # $1=warna_kiri $2=label_kiri "[N]" $3=emoji_kiri $4=teks_kiri
        # $5=warna_kanan $6=label_kanan $7=emoji_kanan $8=teks_kanan
        local CL="$1" LL="$2" EL="$3" TL="$4"
        local CR="$5" LR2="$6" ER="$7" TR="$8"
        local padL=$(( 16 - ${#TL} )); [[ $padL -lt 0 ]] && padL=0
        local padR=$(( 16 - ${#TR} )); [[ $padR -lt 0 ]] && padR=0
        printf "  ${A1}║${NC} ${CL}${LL}${NC}  ${EL}  ${TL}%*s ${A1}║${NC} ${CR}${LR2}${NC}  ${ER}  ${TR}%*s ${A1}║${NC}\n" \
               "$padL" "" "$padR" ""
    }

    # Baris khusus tanpa emoji (untuk [0] Keluar dan footer)
    _row2s() {
        # $1=warna_kiri $2=teks_kiri_penuh $3=vis_kiri
        # $4=warna_kanan $5=teks_kanan_penuh $6=vis_kanan
        local CL="$1" TL="$2" VL="$3"
        local CR="$4" TR="$5" VR="$6"
        local padL=$(( 25 - VL )); [[ $padL -lt 0 ]] && padL=0
        local padR=$(( 25 - VR )); [[ $padR -lt 0 ]] && padR=0
        printf "  ${A1}║${NC} ${CL}${TL}${NC}%*s ${A1}║${NC} ${CR}${TR}${NC}%*s ${A1}║${NC}\n" \
               "$padL" "" "$padR" ""
    }

    # Border helpers — 27 karakter per kolom
    _top2()  { printf "  ${A1}╔%s╦%s╗${NC}\n" "$(printf '═%.0s' {1..27})" "$(printf '═%.0s' {1..27})"; }
    _bot2()  { printf "  ${A1}╚%s╩%s╝${NC}\n" "$(printf '═%.0s' {1..27})" "$(printf '═%.0s' {1..27})"; }
    _sep2()  { printf "  ${A1}╠%s╬%s╣${NC}\n" "$(printf '═%.0s' {1..27})" "$(printf '═%.0s' {1..27})"; }
    _sep2d() { printf "  ${A1}╠%s╫%s╣${NC}\n" "$(printf '╌%.0s' {1..27})" "$(printf '╌%.0s' {1..27})"; }
    ...
    while true; do
        ...
        # ── 2-Column Grid ─────────────────────────────────────
        # Format _row2: warna_kiri label_kiri emoji_kiri teks_kiri  warna_kanan label_kanan emoji_kanan teks_kanan
        # Teks TANPA emoji, emoji dipisah → padding akurat
        _top2
        _row2 "${A2}" "[1]" "👤" "Kelola Akun"   "${A2}" "[2]" "⚙ " "Service"
        _sep2d
        _row2 "${A2}" "[3]" "🤖" "Telegram Bot"  "${A2}" "[4]" "🛒" "Menu Jualan"
        _sep2d
        _row2 "${A2}" "[5]" "📊" "Bandwidth"     "${A2}" "[6]" "🔄" "Restart"
        _sep2d
        _row2 "${A2}" "[7]" "🚀" "Install"       "${A2}" "[8]" "🌐" "Domain"
        _sep2d
        _row2 "${A2}" "[9]" "🎨" "Tema [${THEME_NAME}]"  "${LR}" "[E]" "🗑 " "Uninstall"
        _sep2
        _row2s "${A4}" "[0]  ✖  Keluar"     13  "${DIM}" "OGH-ZIV v1.0"  13
        _bot2
