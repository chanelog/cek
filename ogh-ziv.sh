...
check_os() {
    if [[ ! -f /etc/os-release ]]; then
        echo -e "
[1;31mx OS tidak dikenali! Script ini hanya untuk Debian & Ubuntu.[0m
"
        exit 1
    fi
    ...
    if [[ "$os_name" != "debian" && "$os_name" != "ubuntu" ]] \
       && [[ "$os_like" != *"debian"* && "$os_like" != *"ubuntu"* ]]; then
        echo ""
        echo -e "[1;31m  +------------------------------------------------------+[0m"
        echo -e "[1;31m  |   x  OS TIDAK DIDUKUNG!                              |[0m"
        echo -e "[1;31m  +------------------------------------------------------+[0m"
        echo -e "[1;31m  |[0m  OS kamu : [1;33m${PRETTY_NAME:-$ID}[0m"
        echo -e "[1;31m  +------------------------------------------------------+[0m"
        echo -e "[1;31m  |[0m  Script ini hanya mendukung:                        [1;31m|[0m"
        echo -e "[1;31m  |[0m  [1;32mv[0m  Debian (semua versi)                          [1;31m|[0m"
        echo -e "[1;31m  |[0m  [1;32mv[0m  Ubuntu (semua versi)                          [1;31m|[0m"
        echo -e "[1;31m  +------------------------------------------------------+[0m"
        echo ""
        exit 1
    fi
...
# -- UTILS ---------------------------------------------------
check_root() { [[ $EUID -ne 0 ]] && { echo -e "\n\033[1;31mx Jalankan sebagai root!\033[0m\n"; exit 1; }; }
ok()    { echo -e "  ${A2}v${NC}  $*"; }
inf()   { echo -e "  ${A3}➜${NC}  $*"; }
warn()  { echo -e "  ${A4}⚠${NC}  $*"; }
err()   { echo -e "  \033[1;31mx${NC}  $*"; }
...
draw_vps() {
    ...
    _btn "  ${LG}v IZIN${NC}  ${W}${IZIN_LABEL:--}${NC}  ${A1}|${NC}  ${DIM}EXP${NC} ${_exp_col}${_exp_disp}${NC}  ${A1}|${NC}  ${DIM}IP${NC} ${A3}${IZIN_IP:-?}${NC}"
...
📅 Expired  : <b>$ne</b>  (+${rd} hari)"
    echo ""
    echo -e "  ${A1}+--------------+-------------------------------------------+${NC}"
    echo -e "  ${A1}|${NC}  ${LG}v  Akun berhasil diperpanjang!${NC}                        ${A1}|${NC}"
    echo -e "  ${A1}+--------------+-------------------------------------------+${NC}"
...
u_chpass() {
    ...
    echo ""
    echo -e "  ${A1}+--------------+-------------------------------------------+${NC}"
    echo -e "  ${A1}|${NC}  ${LG}v  Password berhasil diubah!${NC}                          ${A1}|${NC}"
    echo -e "  ${A1}+--------------+-------------------------------------------+${NC}"
    printf  "  ${A1}|${NC} ${DIM} Username  ${NC} ${A1}|${NC}  ${W}%-41s${NC}  ${A1}|${NC}\n" "$pu"
    echo -e "  ${A1}+╌╌╌╌╌╌╌╌╌╌╌╌╌╌+╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌+${NC}"
...
tg_setup() {
    ...
    echo ""
    echo -e "  ${A1}+--------------+-------------------------------------------+${NC}"
    echo -e "  ${A1}|${NC}  ${LG}v  Bot Telegram berhasil dikonfigurasi!${NC}               ${A1}|${NC}"
    echo -e "  ${A1}+--------------+-------------------------------------------+${NC}"
    printf  "  ${A1}|${NC} ${DIM} Bot       ${NC} ${A1}|${NC}  ${W}@%-40s${NC}  ${A1}|${NC}\n" "$bname1"
...
tg_status() {
    ...
    if echo "$res1" | grep -q '"ok":true'; then
        ...
        echo -e "  ${A1}|${NC}  ${LR}🔴  Bot — Tidak Terhubung!${NC}                               ${A1}|${NC}"
        echo -e "  ${A1}+----------------------------------------------------------+${NC}"
    fi
    ...
    if [[ -n "$BOT_TOKEN" && -n "$CHAT_ID" ]]; then
        ...
        if echo "$r1" | grep -q '"ok":true'; then
            ...
            if [[ -n "$fid1" ]]; then
                ...
                ok "Terkirim ke Bot 1 v  (file_id tersimpan)"
            else
                ok "Terkirim ke Bot 1 v  (file_id tidak dapat diambil)"
            fi
            ((sent++))
        else
...
_bak_do_restore() {
    ...
    if tar -xPf "$bpath" 2>/tmp/oghziv_rst_err; then
        ...
        echo ""
        ok "Restore selesai!"
        is_up && ok "ZiVPN ${LG}RUNNING${NC} v" || warn "ZiVPN belum jalan — coba: ${Y}systemctl start zivpn${NC}"
...
u_maxlogin() {
    ...
    echo ""
    echo -e "  ${A1}+--------------+-------------------------------------------+${NC}"
    echo -e "  ${A1}|${NC}  ${LG}v  MaxLogin berhasil diatur!${NC}                          ${A1}|${NC}"
    echo -e "  ${A1}+--------------+-------------------------------------------+${NC}"
    printf  "  ${A1}|${NC} ${DIM} Username  ${NC} ${A1}|${NC}  ${W}%-41s${NC}  ${A1}|${NC}\n" "$mu"
    echo -e "  ${A1}+╌╌╌╌╌╌╌╌╌╌╌╌╌╌+╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌+${NC}"
...
main_menu() {
    ...
    _row2() {
        ...
        local padL=$(( 17 - ${#TL} )); [[ $padL -lt 0 ]] && padL=0
        local padR=$(( 18 - ${#TR} )); [[ $padR -lt 0 ]] && padR=0
        printf "  ${A1}|${NC} ${CL}${LL}${NC}  ${EL}  ${TL}%*s ${A1}|${NC} ${CR}${LR2}${NC}  ${ER}  ${TR}%*s ${A1}|${NC}\n" \
               "$padL" "" "$padR" ""
    }
    # Baris tanpa emoji: kiri pad=27-vis, kanan pad=28-vis
    _row2s() {
        local CL="$1" TL="$2" VL="$3" CR="$4" TR="$5" VR="$6"
        local padL=$(( 27 - VL )); [[ $padL -lt 0 ]] && padL=0
        local padR=$(( 28 - VR )); [[ $padR -lt 0 ]] && padR=0
    ...
    while true; do
        ...
        _sep2
        _row2s "${A4}" "[0]  x  Keluar"  13  "${DIM}" "* OGH-ZIV v1.0"  14
        _bot2
